...
export default function Jadwal() {
    ...
    const fetchData = async (pageNumber = 1, search = '') => {
        ...
        if (token) {
            ...
            try {
                ...
                const response = await Api.get(`/api/jadwal?${queryParams}`);
    ...
    const getStatusBadge = (status) => {
        const statusMap = {
            SCHEDULED: { class: 'bg-info', icon: '📅' },
            IN_PROGRESS: { class: 'bg-warning text-dark', icon: '🔄' },
                ...
                <div className="page-header d-print-none">
                    <div className="container-xl">
                        <div className="row g-2 align-items-center">
                            <div className="col">
                                <h2 className="page-title">Jadwal Pengambilan Sampel</h2>
                                <div className="text-muted mt-1">Penjadwalan pengambilan sampel oleh Sanitarian</div>
                            </div>
                            <div className="col-auto ms-auto d-print-none">
                                <div className="d-flex">
                                    <Link to="/jadwal/create" className="btn btn-primary">
                    ...
                    <div className="container-xl">
                        <div className="card">
                            <div className="card-header">
                                <div className="row g-2 align-items-center">
                                    <div className="col">
                                        <h3 className="card-title">Daftar Jadwal</h3>
                                    </div>
                                        ...
                                        {isLoading ? (
                                            ...
                                            jadwal.map((item, index) => (
                                                <tr key={item.id}>
                                                    ...
                                                    <td className="fw-semibold">{item.order?.no_order || `ORD-${item.order_id}`}</td>
                                                    <td>{item.sanitarian?.name || '-'}</td>
                                                    ...
                                                    <td>
                                                        <div className="btn-group">
                                                            <Link to={`/jadwal/${item.id}`} className="btn btn-sm btn-outline-primary" title="Detail">
                                                            ...
                                                            <Link to={`/jadwal/${item.id}/edit`} className="btn btn-sm btn-outline-secondary" title="Edit">
